...
export default function SkyAdminLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl p-6 sm:p-8 backdrop-blur-xl relative z-10">
        {/* Brand Header */}
        <div className="text-center mb-6 sm:mb-8">
          <img
            src="/images/logo.png"
            alt="Sky Quest Logo"
            className="mx-auto mb-3 h-14 sm:h-16 w-auto object-contain"
          />
          <h1 className="text-2xl font-black text-white tracking-tight">
            <span className="text-sky-400">SKY</span>QUEST <span className="text-amber-400">ADMIN</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Sign in to SkyQuest Operations & Admin Portal
          </p>
        </div>

        {errorMsg && (
          <div className="mb-5 p-3.5 rounded-xl bg-rose-950/50 border border-rose-800 text-rose-300 text-xs flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-bold text-sm shadow-glow flex items-center justify-center gap-2 transition-all disabled:opacity-50 active:scale-[0.98]"
          >
            {loading ? (
              <>
